import { AtendenteCallControls } from '@/components/features/atendente-call-controls';
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        description="Visualize a fila aguardando e chame a próxima senha por prioridade."
      />

      <Card>
        <h2 className="mb-4 text-2xl font-semibold text-slate-900">Ações de chamada</h2>
        <AtendenteCallControls />
      </Card>
      ...
      <section aria-label="Próximas senhas" className="grid gap-4 md:grid-cols-2">
        <Card className="bg-slate-900 text-white">
          ...
          <p className="mt-4 text-6xl font-extrabold tracking-widest">{nextNormal ? formatTicketCode(nextNormal.prefix, nextNormal.ticket_number) : '---'}</p>
        </Card>

        <Card className="bg-brand-700 text-white">
          <h2 className="text-xl font-semibold">Próxima senha preferencial</h2>
          <p className="mt-4 text-6xl font-extrabold tracking-widest">{nextPriority ? formatTicketCode(nextPriority.prefix, nextPriority.ticket_number) : '---'}</p>
        </Card>
      </section>
